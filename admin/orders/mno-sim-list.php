        ...
        try {
            ...
            if ($product) {
                ...
                if ($product['product_type'] === 'mno-sim') {
                    $detailStmt = $pdo->prepare("SELECT * FROM product_mno_sim_details WHERE product_id = :product_id");
                    $detailStmt->execute([':product_id' => $productId]);
                    $detail = $detailStmt->fetch(PDO::FETCH_ASSOC);
                    if ($detail) {
                        $productInfo['mno_sim_details'] = $detail;
                    }
                }
            }
...
if (!$pdo) {
    ...
    // 테이블 존재 여부 확인
    try {
        $tables = ['product_applications', 'application_customers', 'products', 'product_mno_sim_details'];
        $missingTables = [];
...
if ($pdo && empty($errors)) {
    try {
        // 검색 조건 생성
        $whereConditions = ["a.product_type = 'mno-sim'"];
        $params = [];
...
<style>
    ...
    .table-header {
        ...
        display: grid;
        grid-template-columns: 50px 50px 140px 120px 100px 120px 100px 150px 100px 100px 120px 120px 120px 100px 250px 100px;
        gap: 12px;
        font-weight: 600;
        font-size: 13px;
        color: #374151;
    ...
    /* 혜택내용 셀은 잘리지 않도록 */
    .table-row > .table-cell:nth-child(14) {
        overflow: visible;
        white-space: nowrap;
    }
        ...
        ?>
            <div class="table-row">
                ...
                <div class="table-cell">
                    ...
                    ?>
                        <span style="color: #6366f1; font-weight: 600;" title="포인트 사용: <?php echo $formattedPoint; ?>P">
                            <?php echo $formattedPoint; ?>P
                        </span>
                    <?php else: ?>
