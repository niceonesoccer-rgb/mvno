...
#### 사용자 데이터 (DB: `users` 테이블 + 역할별 프로필 테이블)
- **기본 사용자 정보**: `users`
- **판매자 추가 정보**: `seller_profiles`
- **관리자 추가 정보**: `admin_profiles`

#### 포인트 데이터 (DB: `user_point_accounts`, `user_point_ledger`)
- **잔액**: `user_point_accounts.balance`
- **원장**: `user_point_ledger` (delta/type/item_id/description/balance_after)
...
#### 사용자 데이터에 추가할 필드 (선택, DB 컬럼/테이블로 확장)
- **status**: active/inactive/deleted
- **last_login**: 최근 로그인 시간
- **point_balance**: 포인트 잔액(캐시)
- **stats**: 주문/리뷰/Q&A 카운트(캐시 또는 실시간 계산)
...
- `includes/data/auth-functions.php` - 사용자 인증/가입/조회 함수 (DB 기반)
- `includes/data/db-config.php` - DB 연결
...
3. **포인트 정보는 우선**: DB(`user_point_accounts`/`user_point_ledger`) 기반으로 먼저 연동 가능
...
2. 회원 상세 정보
   - 기본 정보 표시
   - SNS 정보 (SNS 가입 시)
   - 판매자 정보 (판매자일 경우, seller_profiles)
   - 포인트 정보 (DB 기반)
```
...
#### 현재 구조의 특징
- **DB 기반**: users/seller_profiles/admin_profiles로 관리
- **확장성**: 검색/필터/집계에 유리
- **동시성**: 트랜잭션으로 정합성 관리 가능

#### 구현 시 주의사항
1. **트랜잭션**: 다중 테이블 업데이트 시 트랜잭션 사용
2. **권한 검증**: 관리자/서브관리자 권한 분리
3. **에러 처리**: DB 예외/제약조건 오류 처리
