...
```php
// 방문 로그 저장
- DB(app_settings 또는 별도 로그 테이블)에 저장
- 페이지뷰, 세션, 사용자 정보 기록
- 통계 데이터 집계 및 표시
```
...
### 2. 통계 데이터 저장
- DB-only: `app_settings(namespace='analytics_data')`에 JSON 형태로 저장 (파일 저장 없음)
...
2. **데이터 저장**
   - DB-only(app_settings) vs 별도 로그 테이블(확장형)
   - 데이터 보관 기간 (예: 최근 1년)
