...
$clickType = isset($data['click_type']) ? trim($data['click_type']) : 'direct';
$targetUrl = isset($data['target_url']) ? trim($data['target_url']) : null;
...
// 클릭 유형 검증
$validClickTypes = ['direct', 'detail', 'apply', 'other'];
if (!in_array($clickType, $validClickTypes)) {
    $clickType = 'direct'; // 기본값
}
...
// 광고 클릭 추적
$result = trackAdvertisementClick($advertisementId, $productId, $sellerId, $productType, $clickType, $targetUrl);

if ($result) {
    sendJsonResponse(['success' => true, 'message' => '광고 클릭이 기록되었습니다.']);
} else {
    sendJsonResponse(['success' => false, 'message' => '광고 클릭 기록에 실패했습니다.'], 500);
}
