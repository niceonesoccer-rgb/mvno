...
try {
    ...
    if ($usedPoint > 0 && $applicationId !== false) {
        $pointType = 'mno-sim';
        if ($pointType === 'mno-sim') {
            $pointType = 'mno'; // 데이터베이스 저장 시 일관성 유지
        }
        $pointDescription = '통신사단독유심 할인혜택';
        $pointResult = deductPoint($userId, $usedPoint, $pointType, $productId, $pointDescription);
        
        if (!$pointResult['success']) {
            error_log("MNO-SIM Application - 포인트 차감 실패: " . ($pointResult['message'] ?? '알 수 없는 오류'));
            // 포인트 차감 실패해도 신청은 완료된 상태이므로 경고만 로그에 기록
        } else {
            error_log("MNO-SIM Application - 포인트 차감 성공: {$usedPoint}포인트 차감됨");
        }
    }
