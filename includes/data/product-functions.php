...
function getProductNumberByType($productId, $productType, $sellerId = null) {
    ...
    try {
        ...
        // 상세 정보가 있는 상품만 카운트 (statistics와 동일한 기준)
        // seller_id가 제공되면 해당 판매자의 상품만 기준으로 계산
        $query = "
        ...
        // 타입별로 상세 정보 테이블과 JOIN하여 상세 정보가 있는 상품만 카운트
        if ($productType === 'mvno') {
            $query = "
                SELECT COUNT(*) + 1 as product_number
                FROM products p
...
/**
 * 15일 이상 미처리 주문 자동 종료 처리
 * 주문, 개통완료, 취소, 설치완료, 종료가 아닌 상태를 15일 후 모두 종료 처리
 * 
 * @return array 처리 결과 ['processed' => 처리된 건수, 'errors' => 오류 개수]
 */
function autoCloseOldApplications() {
    $pdo = getDBConnection();
    if (!$pdo) {
        error_log("autoCloseOldApplications: 데이터베이스 연결 실패");
    ...
    try {
        ...
        if ($processedCount > 0) {
            error_log("autoCloseOldApplications: {$processedCount}건의 주문이 자동으로 종료 처리되었습니다.");
        }
        
        return ['processed' => $processedCount, 'errors' => 0];
        
    } catch (PDOException $e) {
        error_log("autoCloseOldApplications 오류: " . $e->getMessage());
        return ['processed' => 0, 'errors' => 1];
    }
}
...
   *   3 ? ? ? �=�  �����? ? ? .�!? /���  ��d$z�  ? ��y�  ? .�!������  ���% 
   *    
   *   @ r e t u r n   a r r a y   ���%  �[̬��  [ ' p r o c e s s e d '   = >   ���%? ? ? .�!? ? ? ?   ' d e a c t i v a t e d _ p r o d u c t s '   = >   ? .�!������? ? ? ��y�  ? ?   ' e r r o r s '   = >   ? {1��  �Z��Բ]  
   * /  
...
         i f   ( ! $ p d o )   {  
                 e r r o r _ l o g ( " a u t o D e a c t i v a t e I n a c t i v e S e l l e r P r o d u c t s :   ? ׬ �? լ��? ���  ? ̬Ю  ? }1c�" ) ;  
                 r e t u r n   [ ' p r o c e s s e d '   = >   0 ,   ' d e a c t i v a t e d _ p r o d u c t s '   = >   0 ,   ' e r r o r s '   = >   1 ] ;  
         }  
          
         t r y   {  
                 / /   l a s t _ l o g i n   �IF�  p��1�  ? I?   ? ��$� 
                 $ s t m t   =   $ p d o - > q u e r y ( " S H O W   C O L U M N S   F R O M   u s e r s   L I K E   ' l a s t _ l o g i n ' " ) ;  
                 i f   ( $ s t m t - > r o w C o u n t ( )   = = =   0 )   {  
                         e r r o r _ l o g ( " a u t o D e a c t i v a t e I n a c t i v e S e l l e r P r o d u c t s :   l a s t _ l o g i n   �IF�? ? p��1�? �?   ? ���? H���.   a d d _ l a s t _ l o g i n _ c o l u m n . s q l ? ? ? }1��? ��? ����. " ) ;  
                         r e t u r n   [ ' p r o c e s s e d '   = >   0 ,   ' d e a c t i v a t e d _ p r o d u c t s '   = >   0 ,   ' e r r o r s '   = >   1 ] ;  
                 }  
                  
                 / /   3 ? ? ? �=�  �����? ��3�  ? .�!? ? p��v�  ( r o l e = ' s e l l e r ' ? ��,�  l a s t _ l o g i n ? ? N U L L ? ��E�? ? 3 ? ? ? �=�  ��� ? ? �[�æ�)  
                 / /   N U L L ? ? �[�æ�? ? �Z� ? ? ? ? ? ? 0�H�ĸ  �o���? ��/����   ? �?   �[�æ�?  �? �o? c r e a t e d _ a t   rn׬? ? ���  3 ? ? ? �=�  ��� ? ? �[�æ�  ? K?� 
...
                         t r y   {  
                                 / /   ?  ���  ? .�!? /���  ��d$z�  ? ��f�  ? ��y�? ? ? .�!������  ���% 
                                 / /   s e l l e r _ i d   ? � ? ? ? ��$�:   p r o d u c t s . s e l l e r _ i d �Z�   I N T ? ��?   V A R C H A R ? ��? ? ? ? լ*�  ? .�޸  B�� ? �ֹ 
...
                                         e r r o r _ l o g ( " a u t o D e a c t i v a t e I n a c t i v e S e l l e r P r o d u c t s :   ? .�!? ? { $ s e l l e r I d } ? ? { $ d e a c t i v a t e d C o u n t } �Z? ? ��y�? ? ? .�!������  ���%? ���? �r�? ? " ) ;  
                                 }  
                         }   c a t c h   ( P D O E x c e p t i o n   $ e )   {  
                                 e r r o r _ l o g ( " a u t o D e a c t i v a t e I n a c t i v e S e l l e r P r o d u c t s :   ? .�!? ? { $ s e l l e r I d }   ���%  �N? ? {1��  -   "   .   $ e - > g e t M e s s a g e ( ) ) ;  
                         }  
                 }  
                  
                 i f   ( $ t o t a l D e a c t i v a t e d   >   0 )   {  
                         e r r o r _ l o g ( " a u t o D e a c t i v a t e I n a c t i v e S e l l e r P r o d u c t s :   �s? "   .   c o u n t ( $ s e l l e r I d s )   .   " ������  ? .�!? ?   { $ t o t a l D e a c t i v a t e d } �Z? ? ��y�? ? ? .�!������  ���%? ���? �r�? ? " ) ;  
                 }  
                  
                 r e t u r n   [  
...
                  
         }   c a t c h   ( P D O E x c e p t i o n   $ e )   {  
                 e r r o r _ l o g ( " a u t o D e a c t i v a t e I n a c t i v e S e l l e r P r o d u c t s   ? {1��:   "   .   $ e - > g e t M e s s a g e ( ) ) ;  
                 r e t u r n   [ ' p r o c e s s e d '   = >   0 ,   ' d e a c t i v a t e d _ p r o d u c t s '   = >   0 ,   ' e r r o r s '   = >   1 ] ;  
         }  
 }  
 
/**
 * 3???�상 미접???�매?�의 모든 ?�품 ?�매종료 처리
 * 
 * @return array 처리 결과 ['processed' => 처리???�매???? 'deactivated_products' => ?�매종료???�품 ?? 'errors' => ?�류 개수]
 */
function autoDeactivateInactiveSellerProducts() {
    $pdo = getDBConnection();
    if (!$pdo) {
        error_log("autoDeactivateInactiveSellerProducts: ?�이?�베?�스 ?�결 ?�패");
    ...
    try {
        ...
        if ($stmt->rowCount() === 0) {
            error_log("autoDeactivateInactiveSellerProducts: last_login 컬럼??존재?��? ?�습?�다. add_last_login_column.sql???�행?�주?�요.");
            return ['processed' => 0, 'deactivated_products' => 0, 'errors' => 1];
        }
        
        // 3???�상 미접?�한 ?�매??조회 (role='seller'?�고 last_login??NULL?�거??3???�상 지??경우)
        // NULL??경우??가??????번도 로그?�하지 ?��? 경우?��?�?created_at 기�??�로 3???�상 지??경우 ?�함
        $stmt = $pdo->prepare("
        ...
        foreach ($inactiveSellers as $sellerId) {
            try {
                ...
                if ($deactivatedCount > 0) {
                    ...
                    $sellerIds[] = $sellerId;
                    error_log("autoDeactivateInactiveSellerProducts: ?�매??{$sellerId}??{$deactivatedCount}�??�품???�매종료 처리?�었?�니??");
                }
            } catch (PDOException $e) {
                error_log("autoDeactivateInactiveSellerProducts: ?�매??{$sellerId} 처리 �??�류 - " . $e->getMessage());
            }
        }
        
        if ($totalDeactivated > 0) {
            error_log("autoDeactivateInactiveSellerProducts: �?" . count($sellerIds) . "명의 ?�매?? {$totalDeactivated}�??�품???�매종료 처리?�었?�니??");
        }
        ...
        error_log("autoDeactivateInactiveSellerProducts ?�류: " . $e->getMessage());
        return ['processed' => 0, 'deactivated_products' => 0, 'errors' => 1];
    }
}
