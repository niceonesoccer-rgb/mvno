...
if (deleteSellerInquiry($inquiryId, $sellerId)) {
    header('Location: ' . getAssetPath('/seller/inquiry/inquiry-list.php') . '?success=deleted');
} else {
    header('Location: ' . getAssetPath('/seller/inquiry/inquiry-detail.php') . '?id=' . $inquiryId . '&error=cannot_delete');
}
exit;
