...
try {
    ...
    if ($pdo) {
        ...
        $countSql = "
            ...
            INNER JOIN products p ON a.product_id = p.id AND p.product_type = 'mno'
            INNER JOIN product_mno_details mno ON p.id = mno.product_id
            WHERE $whereClause
        ";
        
        // 디버깅 로그
        error_log("MNO Orders - Search Keyword: " . ($searchKeyword ?? 'empty'));
        error_log("MNO Orders - WHERE Clause: " . $whereClause);
        error_log("MNO Orders - COUNT SQL: " . $countSql);
        error_log("MNO Orders - Params: " . json_encode($params));
        
        $countStmt = $pdo->prepare($countSql);
        $countStmt->execute($params);
        $totalOrders = $countStmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;
        
        error_log("MNO Orders - Total Orders: " . $totalOrders);
...
function showProductInfo(order, productType) {
    ...
    if (productType === 'mno') {
        ...
        if (order.used_point && parseInt(order.used_point) > 0) {
            ...
            html += `<tr><th>포인트 사용</th><td style="color: #6366f1; font-weight: 600;">${formattedPoint}P</td></tr>`;
        }
        ...
        if (order.point_benefit_description) {
            ...
            html += `<tr><th>혜택내용</th><td style="color: #10b981; font-weight: 500;">${escapeHtml(order.point_benefit_description)}</td></tr>`;
        }
