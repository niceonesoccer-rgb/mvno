<?php
/**
 * notices 테이블에 start_at과 end_at 필드 추가 스크립트
 * 브라우저에서 실행: http://localhost/MVNO/database/add_notice_dates.php
 */
...
try {
    echo '<h2>notices 테이블에 기간 필드 추가</h2>';
    echo '<style>
    ...
    // 컬럼 존재 여부 확인
    $stmt = $pdo->query("SHOW COLUMNS FROM notices LIKE 'start_at'");
    $startColumnExists = $stmt->rowCount() > 0;
    
    $stmt = $pdo->query("SHOW COLUMNS FROM notices LIKE 'end_at'");
    ...
    if (!$startColumnExists) {
        ...
        $messages[] = '<span class="success">✓ start_at 필드가 성공적으로 추가되었습니다.</span>';
    } else {
        $messages[] = '<span class="info">- start_at 필드가 이미 존재합니다.</span>';
    }
    
    if (!$endColumnExists) {
        $pdo->exec("ALTER TABLE notices ADD COLUMN end_at DATE DEFAULT NULL COMMENT '메인공지 종료일' AFTER start_at");
        $messages[] = '<span class="success">✓ end_at 필드가 성공적으로 추가되었습니다.</span>';
    } else {
        $messages[] = '<span class="info">- end_at 필드가 이미 존재합니다.</span>';
    }
