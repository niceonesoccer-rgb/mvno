<?php
/**
 * 기존 주문번호를 4자리에서 5자리로 업데이트하는 스크립트
 * 형식: YYMMDDHH-0012 -> YYMMDDHH-00012
 */
...
try {
    ...
    // 4자리 형식의 주문번호를 모두 가져오기
    $stmt = $pdo->query("
        SELECT id, order_number 
        FROM product_applications 
        WHERE order_number IS NOT NULL 
        AND order_number REGEXP '^[0-9]{8}-[0-9]{4}$'
        ORDER BY id
    ");
    ...
    foreach ($orders as $order) {
        ...
        // 주문번호 파싱: YYMMDDHH-0012 형식
        if (preg_match('/^(\d{8})-(\d{4})$/', $oldOrderNumber, $matches)) {
            $timePrefix = $matches[1]; // YYMMDDHH
            $sequence = $matches[2];   // 0012
            
            // 순번을 5자리로 변환 (앞에 0 추가)
            $newSequence = str_pad(intval($sequence), 5, '0', STR_PAD_LEFT);
