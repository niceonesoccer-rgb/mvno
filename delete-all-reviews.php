...
try {
    ...
    try {
        $ratingStmt = $pdo->query("
            SELECT 
                COALESCE(SUM(total_rating_sum), 0) as total_rating,
                COALESCE(SUM(kindness_rating_sum), 0) as kindness_rating,
                COALESCE(SUM(speed_rating_sum), 0) as speed_rating
    ...
    $reviewSpeedSum = 0;
    try {
        $reviewRatingStmt = $pdo->query("
            SELECT 
                COALESCE(SUM(rating), 0) as total_rating,
                COALESCE(SUM(kindness_rating), 0) as kindness_rating,
                COALESCE(SUM(speed_rating), 0) as speed_rating
            FROM product_reviews
        ");
        $reviewRatingData = $reviewRatingStmt->fetch();
        $reviewRatingSum = $reviewRatingData['total_rating'] ?? 0;
        $reviewKindnessSum = $reviewRatingData['kindness_rating'] ?? 0;
        $reviewSpeedSum = $reviewRatingData['speed_rating'] ?? 0;
    } catch (PDOException $e) {
        // 오류 무시
    }
    ...
    echo "<h3>별점 통계 (product_review_statistics)</h3>";
    ...
    echo "<hr style='margin: 15px 0; border: none; border-top: 1px solid #ddd;'>";
    echo "<h3>리뷰별 별점 (product_reviews)</h3>";
    echo "<p>기본 별점 합계: <strong>" . number_format($reviewRatingSum, 2) . "</strong></p>";
    echo "<p>친절해요 별점 합계: <strong>" . number_format($reviewKindnessSum, 2) . "</strong></p>";
    echo "<p>설치빨라요 별점 합계: <strong>" . number_format($reviewSpeedSum, 2) . "</strong></p>";
    echo "</div>";
    ...
    if (isset($_POST['confirm_delete']) && $_POST['confirm_delete'] === 'yes') {
        ...
        try {
            ...
            echo "<p>모든 리뷰와 별점 데이터가 성공적으로 삭제되었습니다.</p>";
            echo "</div>";
            ...
            if ($remainingCount == 0 && $remainingStats == 0 && $remainingProductCount == 0 && 
                ...
                echo "<p style='color: green; font-weight: bold; margin-top: 10px;'>✅ 모든 리뷰와 별점 데이터가 완전히 삭제되었습니다!</p>";
            } else {
        ...
        echo "</div>";
            ...
            echo "<form method='POST' onsubmit='return confirm(\"정말로 모든 리뷰와 별점 데이터를 삭제하시겠습니까?\\n\\n- 모든 리뷰 (알뜰폰, 통신사폰, 인터넷)\\n- 모든 별점 통계\\n- 모든 리뷰 카운트\\n\\n이 작업은 되돌릴 수 없습니다.\");'>";
            echo "<input type='hidden' name='confirm_delete' value='yes'>";
            echo "<button type='submit' class='btn'>모든 리뷰 및 별점 삭제</button>";
            echo "</form>";
        } else {
            echo "<div class='info'>";
